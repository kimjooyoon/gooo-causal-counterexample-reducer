...
func LoadContract(path string) (Contract, error) {
	...
	for scanner.Scan() {
		...
		case "oracle":
			...
			if parseErr != nil {
				return Contract{}, fmt.Errorf("line %d: required must be boolean", lineNumber)
			}
			contract.Oracle = OracleContract{Name: get("name"), Required: required, ContractID: get("contract"), ReplayCount: replays}
		case "replay_receipt":
			if len(fields) > 1 {
				contract.ReplayReceiptFields = splitList(fields[1])
			}
		case "claim":
			global, parseErr := strconv.ParseBool(get("global_minimum"))
			if parseErr != nil {
				return Contract{}, fmt.Errorf("line %d: global_minimum must be boolean", lineNumber)
			}
		...
		case "preservation":
			if len(fields) > 1 {
				contract.PreservationFields = splitList(fields[1])
			}
		case "runtime":
			var parseErr error
...
func parseDecisions(value string) []Decision {
	...
	for _, item := range parts {
		if item != "" {
			result = append(result, Decision(item))
		}
	}
	return result
}
...
func (c Contract) Validate() error {
	if c.ID == "" || c.Version == "" {
		return fmt.Errorf("contract identity is required")
	}
	...
	if !sameStrings(c.UnknownFields, wantUnknown) {
		return fmt.Errorf("unknown field contract is not the six-field contract")
	}
	if len(c.Invariants) != 12 {
		return fmt.Errorf("denominator must contain exactly twelve invariants")
	}
	counts := map[string]int{}
	for index, invariant := range c.Invariants {
		if invariant.Ordinal != index+1 || invariant.ID == "" || invariant.Name == "" {
			return fmt.Errorf("invalid invariant ordinal or identity")
		}
		counts[invariant.Group]++
	}
	if counts["FOUNDATION"] != 4 || counts["COHERENCE"] != 4 || counts["REGRESSION"] != 4 {
		return fmt.Errorf("invariant proof partition must be 4/4/4")
	}
	if len(c.Vectors) != 3 {
		return fmt.Errorf("indicator distribution must have three vectors")
	}
	vectorCounts := map[string]int{}
	for _, vector := range c.Vectors {
		vectorCounts[vector.Name] = len(vector.Indicators)
	}
	if vectorCounts["DRIVER"] != 4 || vectorCounts["OUTCOME"] != 4 || vectorCounts["GUARDRAIL"] != 4 {
		return fmt.Errorf("indicator distribution must be 4/4/4")
	}
	if len(c.DeletionOrder) != 5 || c.TieBreak != "lexical_id" {
		return fmt.Errorf("deletion order and lexical tie-break are required")
	}
	for index, rule := range c.DeletionOrder {
		if rule.Ordinal != index+1 || rule.Kind == "" {
			return fmt.Errorf("deletion order must be ordinal and complete")
		}
	}
	...
	if c.GlobalMinimumClaim || !c.PredicateMonotone {
		return fmt.Errorf("contract must disclaim global minimum and declare monotone predicate")
	}
	if c.RuntimeRepositoryWrites != 0 || c.RuntimeLocalTests != 0 || c.RuntimeCrossProjectGates != 0 {
		return fmt.Errorf("runtime zero gates are mandatory")
	}
	if len(c.Scenarios) != 7 {
		return fmt.Errorf("denominator must contain exactly seven canonical scenarios")
	}
	seen := map[string]bool{}
	for index, scenario := range c.Scenarios {
		if scenario.Ordinal != index+1 || scenario.ID == "" || scenario.Fixture == "" || !validDecision(scenario.Expected) {
			return fmt.Errorf("invalid scenario declaration")
		}
		if seen[scenario.ID] {
